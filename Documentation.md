...
- `app.js`: application logic, data loading, UI state, and gameplay flow.
...
- `Play Mode` guides the user through:
  ...
  2. Character selection (two characters)
...
## Data notes
Each JSON file includes example records so you can add the actual game data yourself. Currently includes:
- 2 scenarios (one with an NPC)
- 4 characters across 2 factions (House Velours: 2 nobles, House Aurelian: 2 nobles)
- 4 abilities
- 1 action
- 1 event
...
- `actions.json` includes both standard actions and character-specific abilities as separate items.
